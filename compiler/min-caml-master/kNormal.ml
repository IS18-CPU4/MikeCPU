...
let rec is_2n_loop i = (* i = 2**nかの判定 *)
  if i <= 0 then (* iが負ではダメ *)
    false
  else if i = 1 then
    true
  else if i mod 2 = 0 then (* iが偶数 *)
    is_2n_loop (i/2)
  else
    false

let is_2n = function (* Int(n)のlog2をとる *)
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax2.Mul(e1, e2) -> (* 掛け算のK正規化 (caml2html: knormal_add) *)
      if is_2n e2 then
        insert_let (g env e1)
  ...
  | Syntax2.If(e1, e2, e3) ->
     (match g_fless_feq env e1 with
      | Syntax2.Not _
      | Syntax2.LE _
      | Syntax2.Eq _ as cmp -> g env (Syntax2.If(cmp, e2, e3)) (* fless, feq の時の特別処理 *)

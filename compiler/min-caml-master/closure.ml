...
let rec g env known depth = function (* クロージャ変換ルーチン本体 (caml2html: closure_g) *)
  | KNormal.Unit -> Unit
  | KNormal.Int(i) -> Int(i)
  ...
  | KNormal.LetRec({ KNormal.name = (x, t); KNormal.args = yts; KNormal.body = e1 }, e2) -> (* 関数定義の場合 (caml2html: closure_letrec) *)
      ...
      let env' = M.add_list !ext_arrays_env (M.add x t env) in
      let known' = (S.add x known) in
      let e1' = g (M.add_list yts env') known' (depth + 1) e1 in
      (* 本当に自由変数がなかったか、変換結果e1'を確認する *)
      (* 注意: e1'にx自身が変数として出現する場合はclosureが必要!
         (thanks to nuevo-namasute and azounoman; test/cls-bug2.ml参照) *)
      let zs = S.diff (fv_remove_extarray e1') (S.of_list (List.map fst yts)) in
      let known', e1' =
        ...
        (Format.eprintf "free variable(s) %s found in function %s@." (Id.pp_list (S.elements zs)) x;
         ...
         let e1' = g (M.add_list yts env') known (depth + 1) e1 in
         known, e1') in
      let zs = S.elements (S.diff (fv_remove_extarray e1') (S.add x (S.of_list (List.map fst yts)))) in (* 自由変数のリスト *)
      let zts = List.map (fun z -> (z, M.find z env')) zs in (* ここで自由変数zの型を引くために引数envが必要 *)
      toplevel := { name = (Id.L(x), t); args = yts; formal_fv = zts; body = e1' } :: !toplevel; (* トップレベル関数を追加 *)
      let e2' = g env' known' (depth + 1) e2 in
      if S.mem x (fv_remove_extarray e2') then (* xが変数としてe2'に出現するか *)
